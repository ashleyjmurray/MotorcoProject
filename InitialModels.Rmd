...
```{r crossfold-validation}
#library(rmse)
#test <- mice_full %>%
  #filter(Group == "CV1")
#train <- mice_full %>%
  #filter(Group != "CV1")
#mod <- lm(Attend ~ Capacity + FBFansNC.lt1000 + Touring.Local + Day + Returning.Act + logPSAvgTix + logPSAvgGross + logFBLikes + logTwitter + logSPFollowers + logSPMonthListen, data = train)
...
# Train the model
model <- train(Attend ~ Capacity + FBFansNC.lt1000 + Touring.Local + Day + Returning.Act + logPSAvgTix + logPSAvgGross + logFBLikes + logTwitter + logSPFollowers + logSPMonthListen, data = mice_complete, method = "lm",
