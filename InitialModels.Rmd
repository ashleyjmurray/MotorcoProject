...
```{r interactions}
model_int <- lm(Attend ~ (Capacity + FBFansNC.lt1000 + SPMostPlays.lt1000 + Touring.Local + Day + Returning.Act + logPSAvgTix + logTwitter + logSPFollowers + logYTSubscribers)*Capacity, data = mice_complete)
tidy(model_int)
...
forest_model <- randomForest(Attend ~. - Group - recordID, data = TrainSet, importance = TRUE)
```
